% $Id: testing.erl,v 1.3 2019/10/31 03:54:29 leavens Exp leavens $
-module(testing).
-compile([debug_info]).
...
-export_type([testCase/1,thunkOrNot/1]).
-type thunkOrNot(A) :: {thunk, fun(() -> A)} | A. % see thunk.hrl
-type testCase(A) :: {'test',fun((A,A) -> boolean()),thunkOrNot(A),string(),A}.
...
run_test({test,Comp,{thunk,CodeFun},Connective,Expected}) ->
    CodeRes = CodeFun(),
    run_test({test,Comp,CodeRes,Connective,Expected});
